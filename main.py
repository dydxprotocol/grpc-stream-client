...
from grpc_stream_client.market_info import (
    query_market_info,
    quantums_to_size,
    subticks_to_price,
)
...
async def listen_to_grpc_stream(
    ...
    try:
        ...
        request = StreamOrderbookUpdatesRequest(
            ...
            filter_orders_by_subaccount_id=conf.get("filter_orders_by_subaccount_id", False),
        )
        ...
        logging.exception("gRPC error occurred")
        raise exc
    except Exception as exc:
...
def print_subaccounts(
    ...
    for subaccount_id, subaccount in subaccounts_dict.items():
        perpetual_positions_str = ", ".join(
            [
                f"Perpetual ID: {perp_id}, Quantums: {perp_position.quantums}"
                for perp_id, perp_position in subaccount.perpetual_positions.items()
            ]
        )
        asset_positions_str = ", ".join(
            [
                f"Asset ID: {asset_id}, Quantums: {asset_position.quantums}"
...
async def print_books_every_n_ms(
    feed_handler: FeedHandler,
    market_info_by_clob_pair_id: dict[int, dict],
    interval_ms: int,
):
    """
    ...
    while True:
        ...
        for clob_pair_id, book in feed_handler.get_books().items():
            info = market_info_by_clob_pair_id[clob_pair_id]
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="Sample GRPC Client for Full Node Streaming",
