    ...
    try:
        with open(log_path, 'w') as log:
            async for message in websocket:
                ...
                my_message = StreamOrderbookUpdatesResponse()

                # # Convert the string data to bytes if necessary
                # if isinstance(message, str):
                #     message = message.encode('utf-8')
                #     print(type(message))
                # blah = StreamOrderbookUpdatesResponse.FromString(message)
                # print("serialzied blah", blah)
                # my_message.ParseFromString(message)
                # print("message", my_message)
                # Log the message

                json_format.Parse(message, my_message)
                log.write(my_message + '\n')

                # Update the order book state and print any fills
                try:
                    fill_events = feed_handler.handle(my_message)
                    if conf['print_fills']:
                        print_fills(fill_events, cpid_to_market_info)
                except Exception as e:
                    logging.error(f"Error handling message: {my_message}")
                    raise e
